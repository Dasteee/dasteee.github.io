...
// --- SIMPLE STATION PANEL ---
const StationDetailsPanel = ({ station, linije, buses, onClose }) => {
    const relevantLinije = useMemo(() => {
        if (!station) return [];
        const seen = new Set();
        return linije.filter(l => {
            ...
            ...l,
            finalRouteName: l.routeName.replace(/ - /g, ' ↔ ')
        })).sort((a, b) => a.routeDisplayCode.localeCompare(b.routeDisplayCode, undefined, { numeric: true }));
    }, [station, linije]);
  
    const activeBusesAtStation = useMemo(() => {
        if (!station) return [];
        return buses.filter(bus => {
            const status = getBusStatus(bus.LAST_GPS_TIME);
            if (status.type !== 'Aktivan' || isBusInDepot(bus)) return false;
            
            const relevantLines = linije.filter(l => l.routeCode === bus.ROUTE_CODE && l.stopId === station.STOP_ID);
            return relevantLines.length > 0;
        });
    }, [station, buses, linije]);
